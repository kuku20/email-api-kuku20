...
export class TasksUS_ALL_MK_MASS_Service {
  ...
  //  await this.runOneDaycheckOSC_MACDTRendThenFourHourSame()
    ...
    // const dkd = this.sH_Service.getKeysFromLastN(holdingObj)
    ...
    // this.sH_Service.aboveMA50api = `dayago-${this.dayago}-` +this.rundayaogo;
    // await this.webhooksService.delete(this.sH_Service.aboveMA50api); // reset firebase data for the day
    // await this.webhooksService.delete(this.sH_Service.aboveMA50api+'-blowMA200'); // reset firebase data for the day
  ...
  //  await this.runOnly4h()
    ...
    // await this.sH_Service.writeAbove2BillionToFile(this.aboveTarget,`Above_${this.marketTarget}_Billion`);
    // await this.sH_Service.writeAbove2BillionToFile(this.BelowTarget,`Below_${this.marketTarget}_Billion`);
    // this.sH_Service.ListMA50On1day = await this.LocalPLWR.getArrSymbolFFire('${this.sH_Service.aboveMA50api}/alldata/1day')as string[];
    // const x = await this.LocalPLWR.getArrSymbolFFire(`${this.sH_Service.aboveMA50api}/alldata/lastab/1day`)as string[];
    // await this.getMarket(this.sH_Service.ListMA50On1day )
  }
          ...
          if(OscConditionL){
            ...
            if(OscCrossAb){
              ...
              await this.webhooksService.sendSlackNotificationVN(
                ...
                [ticker],
                lastData,
                this.sH_Service.US_DAILY_.OSC_BL,
                'OscCrossAb',
              );
            }
          }
            ...
            const webhookMap = [
              {
                ...
                hook: this.sH_Service.US_DAILY_.MACDCR_50,
              },
              {
                condition: signal.PriceCrMA100 ,
                hook: this.sH_Service.US_DAILY_.MACDCR_100,
              },
              {
                condition: signal.PriceCrMA200 ,
                hook: this.sH_Service.US_DAILY_.MACDCR_200,
              },
            ];
  ...
  async runfullonms(stocklist = DataSymbols.stock_500_symbols){
    ...
    // this.webhooksService.deleteSLChannel([this.sH_Service.Z_US_SL_.HOLDING,this.sH_Service.INTRA_30M_SL_.MACDCR_50,this.sH_Service.INTRA_30M_SL_.MACDCR_100,this.sH_Service.INTRA_30M_SL_.MACDCR_200,this.sH_Service.INTRA_30M_SL_.MACDCR_BL,this.sH_Service.INTRA_30M_SL_.WATCH])

  }

  async runAllWatchLists(stocklist = DataSymbols.allabove500million) {

    const webhooks = [this.sH_Service.US_DAILY_.MACDCR_50, this.sH_Service.US_DAILY_.MACDCR_100,this.sH_Service.US_DAILY_.MACDCR_200,this.sH_Service.INTRA_30M_SL_.WATCH,this.sH_Service.Z_US_SL_.J2DAY,this.sH_Service.Z_US_SL_.J3DAY,this.sH_Service.US_DAILY_.MACDCR_BL,];
